...
| 5 | **Focus glow — the bloom, and the ring is CUT.** The warm amber bloom shipped on both the focused row's leading icon and the selected caticon (`XmbGlow.kt`); reach is a fraction of the icon rather than a dp, because Icon Size is a user setting. The design's 4px white ring and its black drop are **dropped, not deferred**: they were held back only until a uniform art tile existed, item 9 delivered one, and on seeing it the verdict was *"It's fine as it is now? I don't see the need for that"* (2026-09-24). | `XmbGlow.kt`, `XMBItemList.kt` |
